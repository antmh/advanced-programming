...
public class GenreRepository implements Repository<Genre> {
    private static final String CREATE_STATEMENT = "INSERT INTO genres (name) VALUES (?)";
    ...
    public void update(Genre item) {
        try {
            ...
            statement.setString(1, item.getName());
            statement.setInt(2, item.getId().get());
            statement.execute();
        } catch (SQLException e) {
            System.err.println(e);
        }
    ...
    public Optional<Genre> findByName(String name) {
        try {
            ...
            while (results.next()) {
                Genre genre = new Genre();
                genre.setId(Optional.of(results.getInt("id")));
                genre.setName(name);
                return Optional.of(genre);
            }
        } catch (SQLException e) {
